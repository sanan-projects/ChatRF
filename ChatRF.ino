...
void loop() {
    webSocket.loop(); // WebSocket döngüsünü çalıştır
    server.handleClient(); // Web sunucusunu dinle

    // NRF24L01'den mesajı al
    if (radio.available()) {
...
// Buffer'daki mesajları FIFO sırasıyla gönder
...
// Mesajı göndermeyi dener, eğer şu an mesaj alınıyorsa buffer’a ekler
...
void webSocketEvent(uint8_t num, WStype_t type, uint8_t * payload, size_t length) {
    switch (type) {
        ...
        case WStype_DISCONNECTED:
            ...
            break;
        case WStype_TEXT:
            // Web arayüzünden gelen mesajı al
            String message = String((char*)payload);
            
            if (message == "GET_CHANNEL") {
                // Mevcut kanalı tarayıcıya geri gönder
                webSocket.broadcastTXT("CHANNEL:" + String(channel));
                ...
                // Eğer şu anda karşıdan mesaj alınıyorsa buffer'a ekle
                if (receivingMessage) {
...
void handleSettings(String settings) {
    // SSID, password, SSID broadcast ve channel ayarlarını değiştir
...
void handleChannelSettings(String channelSetting) {
    ...
    // Kanal değişmeden önce karşı tarafa mevcut kanaldan kanal değişikliği sinyali gönder
